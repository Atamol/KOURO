extends SceneTree
## Dev tool: counts which test throws a candidate out, for one level. Run with
##   godot_console --headless --path . -s res://tools/why.gd -- <level index>
	...
	for _placement in 400:
		...
		for _shot in ProblemGen.SOURCE_TRIES:
			...
			if win.events < level.min_events:
				...
				continue
			if min_clear > 0.0 and ProblemGen._too_marginal(result.exits, level.answers, min_clear):
			...
			if level.get("require_split", false) and not ProblemGen._split_by_crystal(result.exits, answers):
				...
				continue
			if level.get("require_rotary", false) and not ProblemGen._path_has_rotary(win.touched, objects):
				_bump(tally, "no rotary on path")
				continue
			...
			if win.tir < level.min_tir or win.split < level.get("min_split", 0):
				_bump(tally, "no tir / no split")
				continue
			if ProblemGen._path_sheets(win.touched, objects) < level.get("min_sheets", 0):
				_bump(tally, "too few sheets on path")
				continue
