...
# collapses under float32 rounding at grazing angles
const ADVANCE := 1e-3
...
const MISTAKES := [
	...
	"no_polarizer", "flip_polarizer", "no_rotation", "flat_gradient", "flip_gradient",
]
...
static func trace(objects: Array, field: Rect2, src_p: Vector2, src_d: Vector2, opts: Dictionary = {}) -> Dictionary:
	...
	var events_total := 0
	var overflow := false
	var start := {"events": 0, "touched": 0, "tir": 0, "split": 0, "clear": INF, "last": -1}
	var queue: Array = [_ray(src_p + src_d * ADVANCE, src_d, src_d, OpticsMaterials.AIR, 1.0, null, "", Jones.unpolarized(), 0.0, start)]
	while not queue.is_empty():
		...
		if ray.inside is GradientBody and mistake != "flat_gradient":
			...
			if hit.is_empty() or (not border.is_empty() and border.t < hit.t):
				...
				segments.append({"a": ray.p, "b": border.point, "intensity": ray.intensity, "t0": ray.time, "t1": t1})
				var out_clear: float = ray.clear
				if measure:
					out_clear = minf(out_clear, _clearance(ray.p, border.point, objects, -1, ray.last))
				exits.append({
					...
					"clear": out_clear, "pol": ray.pol, "jones": _turned(ray, border.t, mistake),
				})
				continue
		events_total += 1
		...
		if measure:
			moved.clear = minf(ray.clear, minf(hit.get("margin", INF),
					_clearance(ray.p, hit.point, objects, hit_index, ray.last)))
		if hit.has("dir"):
			moved.d = hit.dir
			moved.wave = hit.dir
			moved.n_med = hit.n
		if hit_obj.is_polarizer():
			...
			if ti >= min_intensity:
				queue.append(_ray(hit.point - hit.normal * ADVANCE, moved.d, moved.wave, moved.n_med, ti,
						moved.inside, moved.pol, Jones.normalized(Jones.apply(m, moved.jones)), t_hit, _next(moved, touched, false)))
			continue
		if hit_obj.is_reflector():
			...
			if ri >= min_intensity:
				...
				queue.append(_ray(hit.point + side * ADVANCE, rdir, rdir, moved.n_med, ri, moved.inside, moved.pol, turned, t_hit, _next(moved, touched, false)))
			continue
		queue.append_array(_at_interface(moved, hit, hit_obj, touched, t_hit, fresnel, min_intensity, mistake))
	exits.sort_custom(func(x, y): return x.intensity > y.intensity)
	return {"ok": not overflow and not exits.is_empty(), "segments": segments, "exits": exits, "events": events_total, "overflow": overflow}


static func _emit(segments: Array, ray: Dictionary, hit: Dictionary, t_hit: float) -> void:
	if not hit.has("points"):
		segments.append({"a": ray.p, "b": hit.point, "intensity": ray.intensity, "t0": ray.time, "t1": t_hit})
		return
	...
	for i in range(1, pts.size()):
		segments.append({"a": pts[i - 1], "b": pts[i], "intensity": ray.intensity,
				"t0": ray.time + times[i - 1], "t1": ray.time + times[i]})


static func _turned(ray: Dictionary, length: float, mistake: String) -> Array:
	if mistake == "no_rotation" or ray.inside == null:
		return ray.jones
	...
	while travelled < GRIN_MAX:
		...
		if leaving:
			...
			return {
				...
				"normal": _face_normal(body, p, dir), "points": points, "times": times,
			}
	return {}


## Turned to face the ray, the way an intersection would report it
static func _face_normal(body: PolyBody, point: Vector2, dir: Vector2) -> Vector2:
	...
	for i in pts.size():
		...
		var away := Geometry2D.get_closest_point_to_segment(point, a, b).distance_to(point)
		if away < best:
			best = away
			var edge := b - a
			normal = Vector2(-edge.y, edge.x).normalized()
	return -normal if normal.dot(dir) > 0.0 else normal
...
static func _at_interface(ray: Dictionary, hit: Dictionary, body: SceneObj, touched: int, t_hit: float, fresnel: bool, min_intensity: float, mistake := "") -> Array:
	...
	if splitting:
		...
		for mode: Array in [["s", share_s, amp[0]], ["p", 1.0 - share_s, amp[1]]]:
			...
			if ti >= min_intensity:
				out.append(_ray(hit.point - normal * ADVANCE, beam.d, beam.wave, beam.n, ti,
						body, mode[0], Jones.pure(mode[0]), t_hit, _next(ray, touched, false)))
		...
		if ti >= min_intensity:
			...
			out.append(_ray(hit.point - normal * ADVANCE, beam.d, beam.wave, beam.n, ti,
					body if entering else null, ray.pol, state, t_hit, _next(ray, touched, false)))
	...
	if back >= min_intensity:
		...
		if not bounce.is_empty():
			out.append(_ray(hit.point + normal * ADVANCE, bounce.d, bounce.wave, bounce.n, back,
					...
					_next(ray, touched, out.is_empty(), not out.is_empty())))
	return out
...
static func _next(ray: Dictionary, touched: int, tir: bool, split := false) -> Dictionary:
	return {
		"events": ray.events + 1, "touched": touched,
		"tir": ray.tir + (1 if tir else 0), "split": ray.split + (1 if split else 0),
		"clear": ray.clear, "last": ray.last,
	}
...
static func _ray(p: Vector2, d: Vector2, wave: Vector2, n_med: float, intensity: float, inside, pol: String, jones: Array, time: float, path: Dictionary) -> Dictionary:
	# a spawn within ~ADVANCE of a vertex can land outside the body it is
	# flagged inside of. Demote to air so the medium never desyncs
	if inside != null and not inside.contains(p):
		inside = null
		n_med = OpticsMaterials.AIR
		wave = d
	return {
		"p": p, "d": d, "wave": wave, "n_med": n_med, "intensity": intensity, "inside": inside,
		"pol": pol, "jones": jones, "time": time,
		"events": path.events, "touched": path.touched, "tir": path.tir, "split": path.split,
		"clear": path.clear, "last": path.last,
	}
